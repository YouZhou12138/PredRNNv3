...
class KTHDataset(Dataset):
    ...
    def extract_number(self, file_name):
        match = re.search(r"image-(\d+)_128x128", file_name)
        if match:
    ...
    def load_data(self):
        ...
        for c_dir in self.category:
            ...
            c_dir_path = os.path.join(self.base_dir, "kth", c_dir)
...
class KTH(pl.LightningDataModule):
    ...
    def add_data_specific_args(parent_parser: Optional[Union[argparse.ArgumentParser, list]] = None):
        ...
        parser.add_argument('--target_length', default=10, type=int, help='Output sequence length for train')
