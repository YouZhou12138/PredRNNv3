...
class Vision(nn.Module):
    def __init__(self, save_dir, target_length, data_name):
        super().__init__()
    ...
    def forward(self, preds, batch):
        ...
        if self.data_name in ["moving_mnist", "kth", "kth_action"]:
            error_save_dir = os.path.join(self.save_dir, f"error")
            os.makedirs(error_save_dir, exist_ok=True)
            error = torch.abs(targ[-self.target_length:] - pred[-self.target_length:])
            ...
            for t in range(error.shape[0]):
                ...
                img = (img[0] * 255).byte()  # 只取第一个通道
                # 保存为灰度图像
                plt.imsave(f"{error_save_dir}/error_{t}.png", img, cmap='gray')
            ...
            error = np.abs(targ_numpy[-self.target_length:,].astype(np.float32) - pred_numpy[-self.target_length:,].astype(np.float32))

            for t in range(targ_numpy.shape[0]):
                # 获取当前时间点的目标图像

                targ_img = targ_numpy[t, 1, :, :]  # 假设通道数为1
